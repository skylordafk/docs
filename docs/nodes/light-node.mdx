...
<pre><code>
$ celestia light init --p2p.network mocha<br/>
2022-12-19T21:45:00.802Z        INFO    node    nodebuilder/init.go:19  Initializing Light Node Store over '/root/.celestia-light-{constants.mochaChainId}'<br/>
2022-12-19T21:45:00.803Z        INFO    node    nodebuilder/init.go:50  Saving config   {`{"path": "/root/.celestia-light-${constants.mochaChainId}/config.toml"}`}`<br/>
...
```sh
celestia light start --core.ip rpc-blockspacerace.pops.one --p2p.network blockspacerace
```
...
```sh
celestia light start --core.ip <ip-address> --keyring.accname <key_name> --p2p.network blockspacerace
```
...
```sh
celestia light start --core.ip <ip-address> --keyring.accname <name_of_custom_key> --p2p.network blockspacerace
```
...
submitting `PayForBlobs` transactions [here](../../developers/node-tutorial).
